...
public abstract class AbstractRenderer<T extends IChemObject> {
    ...
    public Rect calculateScreenBounds(Rect modelBounds) {
        ...
        Point modelScreenCenter
            = this.toScreenCoordinates(modelBounds.exactCenterX(),
                                       modelBounds.exactCenterY());
        double width = (scale * zoom * modelBounds.width()) + (2 * margin);
        double height = (scale * zoom * modelBounds.height()) + (2 * margin);
        return new Rect((int) (modelScreenCenter.x - width / 2),
    ...
    protected Rectangle convertToDiagramBounds(Rectangle2D modelBounds) {
        ...
        Point screenCoord = this.toScreenCoordinates(xCenter, yCenter);
